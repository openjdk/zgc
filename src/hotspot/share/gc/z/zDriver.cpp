...
private:
  bool should_clear_soft_reference(GCCause::Cause cause) const {
    const bool clear = ZCollectedHeap::heap()->collector_policy()->should_clear_all_soft_refs();
  ...
  void clear_should_clear_soft_reference() const {
    ZCollectedHeap::heap()->collector_policy()->set_should_clear_all_soft_refs(false);
  }
