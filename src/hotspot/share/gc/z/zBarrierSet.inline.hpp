...
template <DecoratorSet decorators, typename BarrierSetT>
template <typename T>
inline bool ZBarrierSet::AccessBarrier<decorators, BarrierSetT>::oop_arraycopy_in_heap(arrayOop src_obj, arrayOop dst_obj, T* src, T* dst, size_t length) {
  if (!HasDecorator<decorators, ARRAYCOPY_CHECKCAST>::value) {
    // No check cast, bulk barrier and bulk copy
    ZBarrier::load_barrier_on_oop_array(src, length);
    return Raw::oop_arraycopy_in_heap(src_obj, dst_obj, src, dst, length);
  }
  ...
  for (const T* const end = src + length; src < end; src++, dst++) {
    ...
    if (!dst_klass->is_instanceof_or_null(elem)) {
      // Check cast failed
      return false;
    }
