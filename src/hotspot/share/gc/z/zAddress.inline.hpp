...
#include OS_CPU_HEADER_INLINE(zAddress)
...
inline bool ZAddress::is_good_or_null(uintptr_t value) {
  ...
  // Assert below disabled for now. This check fails when doing JNIHandles::weak_oops_do()
  // which can walk over "oops" which aren't really oops but instead pointers in the handles
  // free list, which is native memory. I suspect the right solution is to modify
  // JNIHandles::weak_oops_do() to filter out such oops to make sure we never try to pass
  // them through a barrier.
  //
  // assert((is_good(addr) || is_null(addr)) == result, "Bad address");
  return result;
}
