...
void ZPhysicalMemory::add_segment(const ZPhysicalMemorySegment& segment) {
  ...
  if (_nsegments > 0) {
    ZPhysicalMemorySegment& last_segment = _segments[_nsegments - 1];
    assert(last_segment.end() <= segment.start(), "Segments added out of order");
    if (last_segment.end() == segment.start()) {
      last_segment = ZPhysicalMemorySegment(last_segment.start(), last_segment.size() + segment.size());
      return;
    }
  }
