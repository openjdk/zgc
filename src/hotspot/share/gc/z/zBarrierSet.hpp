...
protected:
  // Old BarrierSet interface, remove in the future
  virtual void write_ref_array_work(MemRegion mr) {}
  virtual void write_region_work(MemRegion mr) {}

public:
  ZBarrierSet() : BarrierSet(BarrierSet::FakeRtti(BarrierSet::Z)) {}
  ~ZBarrierSet() {}

  // Old BarrierSet interface, remove in the future
  virtual void resize_covered_region(MemRegion new_region) {}
  virtual bool is_aligned(HeapWord* addr) { return false; }
  virtual void print_on(outputStream* st) const {}

  // New BarrierSet interface
...
template<> struct BarrierSet::GetName<ZBarrierSet> {
  static const BarrierSet::Name value = BarrierSet::Z;
};

template<> struct BarrierSet::GetType<BarrierSet::Z> {
