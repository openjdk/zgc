...
HeapWord* ZObjectAllocator::good_or_null(HeapWord* addr) const {
  return (HeapWord*)ZAddress::good_or_null((uintptr_t)addr);
}
...
void ZObjectAllocator::remap_tlabs() {
  ...
  if (UseTLAB) {
    for (JavaThreadIteratorWithHandle iter; JavaThread* thread = iter.next(); ) {
      thread->tlab().set_start(good_or_null(thread->tlab().start()));
      thread->tlab().set_end(good_or_null(thread->tlab().end()));
      thread->tlab().set_top(good_or_null(thread->tlab().top()));
      thread->tlab().set_pf_top(good_or_null(thread->tlab().pf_top()));
    }
  }
}
