...
// post_parse implies that there might be loadbarrers without uses after parsing
...
static int fixup_uses_in_catch(PhaseIdealLoop *phase, Node *start_ctrl, Node *node) {

  if (!phase->has_ctrl(node)) {
    // This node is floating - doesn't need to be cloned.
    assert(node != start_ctrl, "check");
    return 0; // Unwind - 0 successors
  }
  ...
  return no_succs; // unwind - returning number of nodes that were cloned
}
...
static void call_catch_cleanup_one(PhaseIdealLoop* phase, LoadNode* load, Node* ctrl) {
  ...
  for (int i = 0; i < outcnt; i++) {
    ...
    assert(!n->is_LoadBarrier(), "Sanity");
    int removed = fixup_uses_in_catch(phase, ctrl, n);
    if (removed == 0 ) { // if no successor was cloned, progress to next out.
      index++;
    }
  }
  ...
  for (unsigned int i = 0; i  < load->outcnt(); i++) {
    ...
    // For each load use, see witch catch projs dominates, create load clone lazily and reconnect
    ...
    if (!found_dominating_catchproj) {
      ...
      // Create a phi node that will collect all cloned loads and feed it to the use.
...
void ZBarrierSetC2::barrier_insertion_phase(PhaseIterGVN& igvn) const {
  PhaseIdealLoop ideal_loop(igvn, LoopOptsBarrierInsertion);
  igvn.C->clear_major_progress();
}

void ZBarrierSetC2::barrier_insertion(PhaseIdealLoop* phase) const {
...
void ZBarrierSetC2::insert_load_barriers(PhaseIdealLoop* phase) const {
  ...
  for (GrowableArrayIterator<LoadNode *> loadIter = loadList.begin(); loadIter != loadList.end(); ++loadIter) {
    ...
    } while (!load->is_barrier_expanded());
  }
...
LoadNode* ZBarrierSetC2::insert_one_loadbarrier(PhaseIdealLoop* phase, LoadNode* start_load, Node* ctrl) const {
  ...
  while(!nodestack.is_empty()) {
    ...
    if (n->is_Load() && n != start_load && n->as_Load()->is_barrier_required() && !n->as_Load()->is_barrier_expanded()) {
...
void ZBarrierSetC2::insert_one_loadbarrier_inner(PhaseIdealLoop* phase, LoadNode* load, Node* ctrl, VectorSet visited2) const {
  ...
  // create barrier
  Node* barrier = new LoadBarrierNode(C, NULL, load->in(LoadNode::Memory), NULL, load->in(LoadNode::Address), load->is_barrier_weak());
  Node* barrier_val = new ProjNode(barrier, LoadBarrierNode::Oop);
  Node* barrier_ctrl = new ProjNode(barrier, LoadBarrierNode::Control);
