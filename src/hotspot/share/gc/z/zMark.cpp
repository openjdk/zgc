...
void ZMark::push_partial_array(uintptr_t addr, size_t size, bool finalizable) {
  ...
  Thread::current()->zstacks()->push(&_allocator, &_stripes, stripe, entry, false /* publish */);
}
...
void ZMark::work(uint64_t timeout_in_millis) {
  ...
  ZMarkThreadLocalStacks* const stacks = Thread::current()->zstacks();
...
bool ZMark::flush_and_free(Thread* thread) {
  ZMarkThreadLocalStacks* const stacks = thread->zstacks();
...
public:
  ...
  void do_thread(Thread* thread) {
    guarantee(thread->zstacks()->is_empty(_stripes), "Should be empty");
  }
};
