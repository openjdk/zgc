...
volatile oop* ZReferenceProcessor::reference_referent_addr(oop obj) const {
  return (volatile oop*)java_lang_ref_Reference::referent_addr(obj);
}
...
oop* ZReferenceProcessor::keep(oop obj, ReferenceType type) {
  log_trace(gc, ref)("Pending Reference: " PTR_FORMAT " (%s)", p2i(obj), ReferenceTypeName[type]);
  ...
  oop* const next_addr = (oop*)java_lang_ref_Reference::next_addr(obj);
  Atomic::cmpxchg(obj, next_addr, oop(NULL));

  // Return next in list
  return (oop*)java_lang_ref_Reference::discovered_addr(obj);
}
