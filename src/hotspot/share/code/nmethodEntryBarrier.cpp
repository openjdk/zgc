...
NMethodEntryBarrier::StubEntry::StubEntry(nmethod* nm)
  : _barrier(BarrierSet::barrier_set()->nmethod_entry_barrier()),
    _is_deoptimized(false),
    _nm(nm),
    _return_address_ptr(NULL) { }

NMethodEntryBarrier::StubEntry::StubEntry(nmethod* nm, address* return_address_ptr)
  : _barrier(BarrierSet::barrier_set()->nmethod_entry_barrier()),
    _is_deoptimized(false),
    _nm(nm),
...
void NMethodEntryBarrier::StubEntry::disarm_barrier() {
  assert(!_is_deoptimized, "can only disarm methods that didn't deoptimize!");
  _barrier->disarm_barrier(_nm);
}

bool NMethodEntryBarrier::supports_entry_barrier(nmethod* nm) {
...
int NMethodEntryBarrier::nmethod_stub_entry_barrier(address* return_address_ptr) {
  ...
  StubEntry entry(nm, return_address_ptr);

  assert(!nm->is_osr_method(), "Should not reach here");
  // Called upon first entry after being armed
  bool may_enter = eb->nmethod_entry_barrier(nm);
  if (!may_enter) {
    log_trace(nmethod_barrier)("Deoptimizing nmethod: " PTR_FORMAT, p2i(nm));
    entry.deoptimize();
  }
  return may_enter ? 0 : 1;
}

bool NMethodEntryBarrier::nmethod_osr_entry_barrier(nmethod* nm) {
  ...
  assert(nm->is_osr_method(), "Should not reach here");
  log_trace(nmethod_barrier)("Running osr nmethod entry barrier: " PTR_FORMAT, p2i(nm));
