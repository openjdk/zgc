...
template <typename T> size_t oopDesc::field_offset(T* p) const {
  return pointer_delta((void*)p, (void*)this, 1);
}
...
inline oop  oopDesc::obj_field_weak(int offset) const               { return HeapAccess<AS_NO_KEEPALIVE>::oop_load_at(as_oop(), offset); }
