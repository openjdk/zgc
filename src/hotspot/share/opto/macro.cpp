...
void PhaseMacroExpand::expand_loadbarrier_basic(LoadBarrierNode *barrier)
{
  ...
  Node* call;
  if (barrier->is_weak()) {
    call = new CallLeafNode(tf, CAST_FROM_FN_PTR(address, SharedRuntime::z_load_barrier_on_weak_oop_field_preloaded), "load_barrier", TypeRawPtr::BOTTOM);
  } else {
    call = new CallLeafNode(tf, CAST_FROM_FN_PTR(address, SharedRuntime::z_load_barrier_on_oop_field_preloaded), "load_barrier", TypeRawPtr::BOTTOM);
  }
...
bool PhaseMacroExpand::expand_macro_nodes() {
  ...
  if (C->load_barrier_count() > 0) {
    ...
    C->verify_load_barriers(false);
    {
      ...
      if (!strcmp(ss.as_string(), " EDU.purdue.cs.bloat.editor.Type::simple")) {
        C->root()->dump(9999);
      }
    }
#endif
