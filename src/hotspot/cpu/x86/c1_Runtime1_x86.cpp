...
#if 0
        // Save context. Note that rax is saved/restored in the stub instead of
        // here since we want to catch the return value from z_load_barrier_on_oop_field_preloaded
        // or z_load_barrier_on_weak_oop_field_preloaded.
        __ push(rcx);
        __ push(rdx);
        __ push(rdi);
        __ push(rsi);
