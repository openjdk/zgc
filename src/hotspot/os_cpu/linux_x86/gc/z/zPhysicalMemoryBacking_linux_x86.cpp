...
size_t ZPhysicalMemoryBacking::commit(size_t size) {
  ...
  while (committed < size) {
    size_t remaining = size - committed;
    const uintptr_t start = _uncommitted.alloc_from_front_at_most(&remaining);
    ...
    // Try commit hole
    const size_t filled = _file.commit(start, remaining);
    if (filled > 0) {
    ...
    if (filled < remaining) {
      ...
      _uncommitted.free(start + filled, remaining - filled);
      return committed;
    }
  }
...
size_t ZPhysicalMemoryBacking::uncommit(size_t size) {
  ...
  while (uncommitted < size) {
    size_t remaining = size - uncommitted;
    const uintptr_t start = _committed.alloc_from_back_at_most(&remaining);
    assert(start != UINTPTR_MAX, "Allocation should never fail");

    // Try punch hole
    const size_t punched = _file.uncommit(start, remaining);
    if (punched > 0) {
      // Successful or partialy successful
      _uncommitted.free(start, punched);
      uncommitted += punched;
    }
    if (punched < remaining) {
      // Failed or partialy failed
      _committed.free(start + punched, remaining - punched);
      return uncommitted;
    }
  }
...
void ZPhysicalMemoryBacking::map_view(const ZPhysicalMemory& pmem, uintptr_t addr, bool pretouch) const {
  ...
  for (size_t i = 0; i < nsegments; i++) {
    ...
    segment_addr += segment.size();
  }

  const size_t size = segment_addr - addr;
