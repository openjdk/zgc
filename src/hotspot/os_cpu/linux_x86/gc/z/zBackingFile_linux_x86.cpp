...
ZErrno ZBackingFile::fallocate_compat_mmmap(size_t offset, size_t length, bool touch) const {
  ...
  if (touch) {
    ...
    for (char* p = start; p < end; p += _block_size) {
      *p = 0;
    }
  }
...
ZErrno ZBackingFile::fallocate_fill_hole_compat(size_t offset, size_t length) const {
  ...
  const ZErrno err = is_hugetlbfs() ? fallocate_compat_mmmap(offset, length, false /* touch */)
...
ZErrno ZBackingFile::fallocate_fill_hole_syscall(size_t offset, size_t length) const {
...
ZErrno ZBackingFile::fallocate_fill_hole(size_t offset, size_t length) const {
  ...
  // associate them to segments of the file. We must guarantee that we at
  // some point touch these segments, otherwise we can not punch hole in them.
  if (is_hugetlbfs() || !z_fallocate_supported) {
    return fallocate_fill_hole_compat(offset, length);
  } else {
    return fallocate_fill_hole_syscall(offset, length);
  }
}

ZErrno ZBackingFile::fallocate_punch_hole(size_t offset, size_t length) const {
  if (is_hugetlbfs()) {
    // Guarantee that we have touched the pages we are about to punch hole in
    const ZErrno err = fallocate_compat_mmmap(offset, length, true /* touch */);
    if (err) {
      // Failed
      return err;
    }
...
ZErrno ZBackingFile::split_and_fallocate(bool punch_hole, size_t offset, size_t length) const {
  // Try first half
...
ZErrno ZBackingFile::fallocate(bool punch_hole, size_t offset, size_t length) const {
